...
function start() {
  ...
  return subscriber.psubscribeAsync(`__keyspace@*__:${longhousePattern}`);
}
...
function handleMessage(_pattern, channel, message) {
  const presenceKey = channel.split(':')[1];
  const eventName   = message;
  const subject     = Client.parsePresenceKey(presenceKey);

  switch (eventName) {
    case 'hset':
      Redis.hgetallAsync(presenceKey).then(client => {
        publishEvent('remote join/update', client);
      });
      break;
    case 'expired':
...
function publishEvent(eventName, subject) {
  const clients = ClientRegister.clientsInSpace(subject.space_id);
