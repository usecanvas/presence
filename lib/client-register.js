...
function registerClient(clientID, socket) {
  ...
  return createClient(clientID, socket).then(_client => {
    ...
    return Redis.setAsync(presenceKey, client.identity, ttl[0], ttl[1]);
  }).then(() => {
...
function renewClient(client) {
  ...
  return Redis
    .setAsync(presenceKey, client.identity, ttl[0], ttl[1])
    .return(client);
}
...
function deregisterClient(client) {
  ...
  return Redis.delAsync(presenceKey).then(() => {
...
/**
 * Get the TTL values for client presence.
 *
 * Getting these for each client makes testing easier.
 *
 * @private
 * @return {Array.<string,number>} An array containing [0] the unit for presence
 *   TTL and [1] the TTL value itself
 */
function getPresenceTTL() {
  const ttlUnit = process.env.PRESENCE_TTL_UNIT || 'EX'; // seconds, 'PX' for ms
  const ttlVal  = parseInt(process.env.PRESENCE_TTL, 10) || 60;
  return [ttlUnit, ttlVal];
}
