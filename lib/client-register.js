...
function registerClient(clientID, socket) {
  ...
  return createClient(clientID, socket).then(_client => {
    ...
    const members = presenceKeys.map(getClientInfoFromPresenceKey)
      .map(client => pick(client, 'id', 'identity'));

    return ClientMessager.send(client, { clients: members });
  }).then(() => client);
}
...
/**
 * Extract an client from a presence key.
 *
 * @private
 * @param {string} presenceKey A presence key
 * @return {Client} A client
 */
function getClientInfoFromPresenceKey(presenceKey) {
  return {
    id: presenceKey.split('.').slice(3, 4).join('.'),
    identity: presenceKey.split('.').slice(4).join('.'),
  };
}
...
/**
 * Remove all client presence keys, since the server is shutting down.
 *
 * @return {Promise} A promise resolved when the keys have been removed
 */
function terminate() {
  const allClients = objectValues(clients);
  Logger.log({ event: 'Terminating all clients' });
  return Redis.delAsync(allClients.map(getPresenceKey));
}
