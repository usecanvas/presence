 ...
 * ClientRegister.registerClient('clientID', webSocket).then(client => console.log(client));
...
function registerClient(clientID, socket) {
  ...
  return Client.create(clientID, socket).then(_client => {
    client = _client;
...
function terminate() {
  ...
  const keys       = allClients.map(Client.getPresenceKey);
