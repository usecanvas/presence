...
module.exports = {
  ...
  handleNotJoined         : handleNotJoined,
...
/**
 * Send a response for when a client who is trying to send an action has not
 * joined.
 * @param {ws.WebSocket} client The client who sent the bad message
 */
function handleNotJoined(client/*, message */) {
  logfmt.log({ error: 'Not joined.' });
  sendMessage(client,
    { errors: [{ detail: 'You have not yet joined a space.' }]});
}
